...
def train_ff_dnn(kaldi_label_file,  # a remplacer par un IdMap pour la définition des segments et un repertoire de fichiers label à charger
                 hidden_layer_sizes, 
                 layer_activations, 
                 output_file_name, 
                 lr=0.008, 
                 segment_buffer_size=80, 
                 batch_size=512, 
                 max_iters=20, 
                 tolerance=0.003):
    """
    :paaram kaldi_label_file: label file generated by Kaldi, senone alignement
        per segment
    ...
    with open(kaldi_label_file, 'r') as inputf:
    ...
    idx = np.random.permutation(len(total_seg_list)).astype('int')
    ...
    file_list = list(set([seg[0] for seg in seg_list]))

    print('Number of files to process: {}'.format(len(file_list)))
    print('Number of segments to process: {}'.format(len(total_seg_list)))

    """ 
    Step 2:

    Compute mean and standard deviation on all training features
    """
    ...
    for filename in file_list[::3]:
        ...
        features = []
        for seg in seg_list:
            if seg[0] == filename and os.path.exists(swb_fb_dir + seg[0] + '.fb'):
                features.append(sidekit.frontend.features.get_trap(sidekit.frontend.io.read_spro4_segment(swb_fb_dir + seg[0] + '.fb', start=seg[1]-left_context, end=seg[2]+right_context), left_ctx=left_context, right_ctx=right_context, dct_nb=dct_nb))
    ...
    """
    segment_sets = [seg_list[i:i+segment_buffer_size] for i  in range(0, len(seg_list), segment_buffer_size)]
    ...
    for kk in range(1,max_iters):
        ...
        for ii, segment_set in enumerate(segment_sets):
            ...
            for idx, val  in enumerate(segment_set):
                ...
                e = s + len(label)
                l.append(label)
                f.append(sidekit.frontend.features.get_trap(sidekit.frontend.io.read_spro4_segment(swb_fb_dir + fn + '.fb', start=s-left_context, end=e+right_context), left_ctx=left_context, right_ctx=right_context, dct_nb=dct_nb))
            ...
            log.info("%d/%d | %f | %f ", nfiles, len(seg_list), error / n, accuracy / n)
        ...
        for ii, segment in enumerate(cv_list):
            ...
            X = sidekit.frontend.features.get_trap(sidekit.frontend.io.read_spro4_segment(swb_fb_dir + fn + '.fb', start=s-left_context, end=e+right_context), left_ctx=left_context, right_ctx=right_context, dct_nb=dct_nb)

            assert len(X) == len(t)
            err, acc = xentropy(X, t)
            error += err; accuracy += acc; n += len(X)
        log.info("%d | %f | %f", len(cv_list), error / n, accuracy / n)
        #np.savez(out_nn_weights+'_epoch'+str(kk), **nn_def.get_params(params_))
